...
func (c *Consul) RegisterService() error {
	...
	if err := client.Agent().ServiceRegister(service); err != nil {
		log.Printf("failed to register inventory service with consul with error: %v\n", err)
		return err
	}

	log.Println("successfully registered inventory microservice with hashicorp consul service discovery")
	return nil
}
